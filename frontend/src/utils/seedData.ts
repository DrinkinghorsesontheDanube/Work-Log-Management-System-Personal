...
export function seedAllData() {
  ...
  const todos: Todo[] = [
    { id: id('todo', 1), title: '完成智慧城市数据治理方案修改稿', description: '根据评审会意见修改方案第三章', status: 'in_progress', priority: 'high', dueDate: daysAgo(-2), projectId: projIdMap.smartcity, planTaskId: null, createdAt: iso(daysAgo(0)), updatedAt: iso(daysAgo(0)) },
    { id: id('todo', 2), title: '应急指挥调度系统投标文件终稿提交', description: '技术部分已完稿，需商务部分配合，25号前提交', status: 'pending', priority: 'high', dueDate: daysAgo(-10), projectId: projIdMap.emergency, planTaskId: null, createdAt: iso(daysAgo(5)), updatedAt: iso(daysAgo(1)) },
    { id: id('todo', 3), title: '教育信息化2.0项目需求调研报告编写', description: '汇总4所学校的调研结果', status: 'in_progress', priority: 'medium', dueDate: daysAgo(-5), projectId: projIdMap.edu, planTaskId: null, createdAt: iso(daysAgo(4)), updatedAt: iso(daysAgo(4)) },
    { id: id('todo', 4), title: '交通集团顶层设计方案第一版', description: '完成现状评估、愿景目标、架构设计三个核心章节', status: 'pending', priority: 'medium', dueDate: daysAgo(-15), projectId: projIdMap.transport, planTaskId: null, createdAt: iso(daysAgo(11)), updatedAt: iso(daysAgo(11)) },
    { id: id('todo', 5), title: '政务云迁移第二批系统评估报告', description: '对剩余18个待迁移系统进行迁移难度和风险评估', status: 'pending', priority: 'medium', dueDate: daysAgo(-8), projectId: projIdMap.cloud, planTaskId: null, createdAt: iso(daysAgo(2)), updatedAt: iso(daysAgo(2)) },
    { id: id('todo', 6), title: '整理Q2季度售前项目台账', description: '汇总本季度所有在跟项目进展和预计签约金额', status: 'completed', priority: 'low', dueDate: daysAgo(3), projectId: null, planTaskId: null, createdAt: iso(daysAgo(10)), updatedAt: iso(daysAgo(3)) },
    { id: id('todo', 7), title: '准备智慧城市项目月度汇报PPT', description: '大数据局要求每月提交项目进展汇报', status: 'pending', priority: 'high', dueDate: daysAgo(-1), projectId: projIdMap.smartcity, planTaskId: null, createdAt: iso(daysAgo(2)), updatedAt: iso(daysAgo(2)) },
    { id: id('todo', 8), title: '跟进卫健委区域医疗平台项目信息', description: '刘处提到下半年有区域医疗信息化新项目立项', status: 'pending', priority: 'medium', dueDate: daysAgo(-20), projectId: null, planTaskId: null, createdAt: iso(daysAgo(10)), updatedAt: iso(daysAgo(10)) },
    { id: id('todo', 9), title: '更新公司政务行业解决方案标准模板V3.2', description: '新增数据安全和等保合规章节', status: 'completed', priority: 'medium', dueDate: daysAgo(5), projectId: null, planTaskId: null, createdAt: iso(daysAgo(15)), updatedAt: iso(daysAgo(5)) },
    { id: id('todo', 10), title: '生态环境监测平台可行性报告', description: '为下周与生态环境局的交流准备技术可行性分析', status: 'pending', priority: 'low', dueDate: daysAgo(-5), projectId: projIdMap.env, planTaskId: null, createdAt: iso(daysAgo(3)), updatedAt: iso(daysAgo(3)) },
    { id: id('todo', 11), title: '智慧医疗项目运维报告提交', description: '按时提交6月份运维月报', status: 'completed', priority: 'low', dueDate: daysAgo(2), projectId: projIdMap.health, planTaskId: null, createdAt: iso(daysAgo(10)), updatedAt: iso(daysAgo(2)) },
    { id: id('todo', 12), title: '完成应急项目标书技术评分标准制定', description: '对标竞品制定差异化评分标准', status: 'in_progress', priority: 'high', dueDate: daysAgo(-3), projectId: projIdMap.emergency, planTaskId: null, createdAt: iso(daysAgo(7)), updatedAt: iso(daysAgo(1)) }
  ]
