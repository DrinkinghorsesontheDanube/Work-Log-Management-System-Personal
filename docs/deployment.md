...
服务器**自动**创建一致性备份（SQLite `VACUUM INTO` 快照，WAL 模式下也完整一致）：

- 触发时机：每次启动、每 24 小时、每次导入数据**之前**（导错可回滚）
- 位置：`data/backups/worklog-<时间戳>.sqlite`，保留最近 **14** 份，自动清理更旧的
- 数据库为空时自动跳过；也可手动触发：登录后 `POST /api/backup`
- 恢复：停服务 → 用备份文件覆盖 `data/worklog.sqlite`（并删除 `-wal`/`-shm`）→ 启动

### 手动备份

- 拷贝 `data/` 目录；或在应用内「系统设置 → 导出数据」导出 JSON（不含 AI API Key）

### 从 v1.1（localStorage 版）升级

用**原来存有数据的浏览器**打开新系统并登录，首次进入会自动把浏览器 localStorage 里的旧数据**合并**迁移到服务器（按 id 合并：同 id 以旧数据为准，服务器上已有的、其他设备新录的数据保留）。迁移标记写入 localStorage（`worklog_migrated_v2`），不会重复执行。

### 多设备并发说明

数据同步按**实体增量**进行（只推送本机新增/修改/删除的条目），多设备同时编辑不会整表互相覆盖；同一实体被两台设备同时修改时，后保存的生效（last-write-wins）。切换回某个设备的浏览器标签页时会自动拉取服务器最新数据。
